...
export default function InboxPage() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const result = await apiFetch<VendorMeEnvelope>(
          '/api/vendors/me',
          undefined,
        ...
        if (result.error || !result.data?.data?.id) {
          logger.error('InboxPage: failed to load vendor', { error: result.error });
          setPageError(result.error ?? 'Failed to load vendor profile.');
          setPageLoading(false);
          return;
        }

        setVendorId(result.data.data.id);
      } catch (err) {
  ...
  useEffect(() => {
    if (!vendorId) return;
    void loadConversations(vendorId);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [vendorId]);

  async function loadConversations(vid: string) {
    ...
    try {
      ...
      // Batch-load contact names from profiles
      ...
      setConversations(rows.map((r) => ({
        ...
        contact_name: nameMap.get(r.user_id) ?? 'User',
      })));
    } catch (err) {
              ...
              conversations.map((conv) => (
                ...
                >
                  <div className="flex items-center justify-between mb-0.5">
                    <p className="text-sm font-medium text-gray-800 truncate">{conv.contact_name}</p>
        ...
        <div className={`flex-1 flex flex-col bg-gray-50 ${selectedConvId ? 'flex' : 'hidden sm:flex'}`}>
          ...
          {selectedConv ? (
            <div className="px-6 py-4 bg-white border-b border-gray-100 flex items-center gap-3">
              ...
              <p className="text-sm font-semibold text-gray-800">{selectedConv.contact_name}</p>
            </div>
          ) : (
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
            {!selectedConvId ? (
              ...
              messages.map((msg) => {
                // isMine: any message NOT from the other user's account is from the vendor
